        ...
        for name in zf.namelist():
            ...
            for enc in encodings:
                try:
                    ...
                    df = df.dropna()
                    df = df[pd.to_numeric(df['Vertical'], errors='coerce').notnull()]
...
def interpolate_and_export(df, aoi_gdf, grid_res, crs_epsg, output_path):
    ...
    print(f"Kriging interpolation grid: {xi.size} x {yi.size} = {xi.size * yi.size} cells")
    OK = OrdinaryKriging(
        x, y, z,
        variogram_model='linear',  # หรือ 'spherical', 'gaussian', 'exponential'
        verbose=False,
...
if __name__ == "__main__":
    ...
    try:
        ...
        interpolate_and_export(df, aoi_gdf, grid_res=10, crs_epsg=3382, output_path=raw_tif_path)
